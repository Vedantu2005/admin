...
interface AddComboProductProps {
    onSaveProduct: (productData: Omit<ComboProduct, 'id' | 'status'>, id: number | null) => void;
    productToEdit: ComboProduct | null;
}
...
const AddComboProduct: React.FC<AddComboProductProps> = ({ onSaveProduct, productToEdit }) => {
    const initialFormState = {
        productName: '', category: '', shortDescription: '', rating: '',
        longDescription: '', actualMRP: 0, sellingMRP: 0, discount: 0,
        ingredients: '', benefits: '', storageInfo: ''
    };
    ...
    const [faqs, setFaqs] = useState<ProductFaq[]>([
        { id: Date.now() + 3, question: 'What is included in this combo?', answer: 'This combo includes a selection of our best-selling oils.' },
        { id: Date.now() + 4, question: 'Is there a discount on this combo pack?', answer: 'Yes, combo packs are offered at a discounted price compared to buying items individually.'}
    ]);
    ...
    const handleReset = () => {
        ...
        setMainImage(null);
        setOtherImages([]);
        setVariants([ { id: Date.now() + 1, bottleSize: 'Standard Combo', actualMRP: 420, sellingMRP: 360, discount: 14, pricePerLiter: 0 }]);
        setFaqs([
            { id: Date.now() + 3, question: 'What is included in this combo?', answer: 'This combo includes a selection of our best-selling oils.' },
            { id: Date.now() + 4, question: 'Is there a discount on this combo pack?', answer: 'Yes, combo packs are offered at a discounted price compared to buying items individually.'}
        ]);
    ...
    <div className="bg-gray-50 p-6 sm:p-0 font-sans">
      <div className="space-y-6">
        ...
        {/* Section 1: Combo Product Details */}
        <div className="bg-white p-6 rounded-lg shadow-sm">
          <h2 className="text-lg font-semibold text-gray-700 mb-4">{productToEdit ? 'Edit Combo Product' : 'Add Combo Product'}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label htmlFor="productName" className="block text-sm font-medium text-gray-600 mb-1">Combo Product Name</label>
              <input type="text" id="productName" value={formState.productName} onChange={handleFormChange} placeholder="Combo Name" className="w-full p-2 border border-[#703102] rounded-md focus:ring-2 focus:ring-[#703102]/50 focus:border-[#703102]" />
            </div>
            <div>
              ...
              <select id="category" value={formState.category} onChange={handleFormChange} className="w-full p-2 border border-[#703102] rounded-md focus:ring-2 focus:ring-[#703102]/50 focus:border-[#703102]">
                ...
                <option value="Healthy Oils Combo">Healthy Oils Combo</option>
                <option value="Cooking Essentials Pack">Cooking Essentials Pack</option>
              </select>
            </div>
        ...
        {/* Section 2: Upload Combo Product Image */}
        <div className="bg-white p-6 rounded-lg shadow-sm">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm">
          <h2 className="text-2xl font-bold text-gray-800 mb-6">Combo Info</h2>
          <div className="space-y-6">
            <div>
              <label htmlFor="ingredients" className="block text-md font-semibold text-gray-700 mb-2">Products Included</label>
              <textarea id="ingredients" value={formState.ingredients} onChange={handleFormChange} placeholder="List the products in this combo" rows={4} className="w-full p-3 border-2 border-[#703102] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#703102]/50"></textarea>
            </div>
            <div>
              <label htmlFor="benefits" className="block text-md font-semibold text-gray-700 mb-2">Benefits</label>
              <textarea id="benefits" value={formState.benefits} onChange={handleFormChange} placeholder="Benefits of this combo" rows={4} className="w-full p-3 border-2 border-[#703102] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#703102]/50"></textarea>
            </div>
            <div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm">
          <h2 className="text-2xl font-bold text-gray-800 mb-6">Combo Product Faqs</h2>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md relative">
              ...
              <div className="space-y-4">
                <div>
                  <label htmlFor="variant-bottle-size" className="block text-md font-semibold text-gray-700 mb-2">Variant Name</label>
                  <select name="bottleSize" value={newVariant.bottleSize} onChange={handleNewVariantChange} className="w-full p-3 border-2 border-[#703102] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#703102]/50">
                    ...
                    <option value="Standard Combo">Standard Combo</option>
                ...
                <div>
                  <label htmlFor="variant-price-per-liter" className="block text-md font-semibold text-gray-700 mb-2">Price / L (Optional)</label>
